...
public class Varasto {
    ...
    public Varasto(double tilavuus, double alkuSaldo) { // kuormitetaan
        if (tilavuus > 0.0) {
            ...
            this.tilavuus = 0.0; // => kÃ¤yttÃ¶kelvoton varasto
        }

        saldoCheck(tilavuus, alkuSaldo);
    }

    public void saldoCheck(double tilavuus, double alkuSaldo) {
        if (alkuSaldo < 0.0) {
            this.saldo = 0.0;
        } else if (alkuSaldo <= tilavuus) { // mahtuu
            this.saldo = alkuSaldo;
        } else {
            this.saldo = tilavuus; // tÃ¤yteen ja ylimÃ¤Ã¤rÃ¤ hukkaan!
        }
    }
